...
public class BrevetDroitDaoImpl implements BrevetDroitDao {
		...
		ResultSet resultat = null;
			...
			try {
				...
				if(etat.equals("attente")) {
					resultat = statement.executeQuery(" SELECT o.*,nom,prenom from brevetsDroits o,utilisateurs u where o.id_utilisateur=u.id_utilisateur and validation=false and cne is null order by id_brevetdroit desc;");
				}else if(etat.equals("valider")) {
					resultat = statement.executeQuery(" SELECT o.*,nom,prenom from brevetsDroits o,utilisateurs u where o.id_utilisateur=u.id_utilisateur and validation=false order by datebrevetdroit desc;");					
				}
				...
				while (resultat.next()) {
					...
					brevetDroit.setPreuve(resultat.getString("preuve"));
	...
	public void modifier(BrevetDroit brevetdroit) {
		...
		try {
			connexion = daoFactory.getConnection();
			preparedStatement = connexion.prepareStatement("update brevetsdroits SET intitule=?,type=?,champApplication=?,numeroDepot=?,numeroenregistrement=?,acteurexterne=?,id_Utilisateur=?,datebrevetdroit=?  WHERE id_brevetdroit=?");
	...
	// #############" valider #########
		 ...
		 * (non-Javadoc)
		 * 
		 * @see com.pfe.dao.BrevetDroit#valider(int) public void valider(int id) valider
		...
		public void valider(int id) {
			...
			try {
				connexion = daoFactory.getConnection();
				preparedStatement = connexion.prepareStatement("UPDATE brevetsdroits SET validation=true where id_brevetdroit=?");
				preparedStatement.setInt(1, id);
